...
[M_train_all, M_test_all] = loadMovieLens1m();
...
for fold=1:5
	...
	%M_train(find(M_train))=M_train(find(M_train))-2.5;
	%M_test(find(M_test))=M_test(find(M_test))-2.5;
	...
	b_predict_ti=(min(max(b_predict_ti, 1),5));
	...
	b_predict_nn=(min(max(b_predict_nn, 1),5));
	opts=[];
	...
	b_predict_LMaFit=(min(max(b_predict_LMaFit, 1),5));
	nmae(fold,1)=mean(abs(full(b_test-b_predict_ti)))/4;
	nmae(fold,2)=mean(abs(full(b_test-b_predict_nn)))/4;
	nmae(fold,3)=mean(abs(full(b_test-b_predict_LMaFit)))/4
